...
function Search(){
    ...
    async function searchdata(): Promise<void> {
        setLoading(true);
        console.log("🔍 Fetching with query:", debouncedSearch);
                        ...
                        <Fragment>
                            ...
                            data.map((item) => (
                                ...
                                <Link   className="flex-1" href={`/anime/info/${item.id}`} onClick={() => useSearchbar.setState({ Isopen: false })} onMouseDown={(e) => e.preventDefault()}>
                                    ...
                                    <span className="my-1 text-xs text-gray-400">Episodes - {item?.episodes || (item?.nextAiringEpisode?.episode ? item.nextAiringEpisode.episode - 1 : "?") || "?"}</span>
                                    <div className="flex items-center text-gray-400 text-xs">
                                        ...
                                        {item.averageScore ? item.averageScore / 10 : "0"}
                                        </span>
