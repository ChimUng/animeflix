...
const Overview: React.FC<OverviewProps> = ({ data }) => {
    ...
    return (
        ...
        <div className={styles.card1}>
            ...
            <div className={styles.singlecontent}>
                <span className={styles.sideheading}>Loại</span>
                <span className={`${styles.con}`}>{data?.format}</span>
            </div>
            <div className={styles.singlecontent}>
            ...
            <div className={styles.singlecontent}>
                ...
                <span className={`${styles.conAccent}`}>{data?.genres?.join(', ')}</span>
            </div>
            <div className={styles.singlecontent}>
                ...
                {data?.endDate?.day && (
                    ...
                    {' '}to{' '}
                    <div>
                        {data.endDate.month && aired(data.endDate.month - 1)} {data.endDate.day}, {data.endDate.year}
                    </div>
                    </>
                )}
                </span>
            ...
            <div className={styles.singlecontent}>
                ...
                <span className={`${styles.conAccent}`}>{`${data?.season ?? ''} ${data?.seasonYear ?? ''}`}</span>
            </div>
            <div className={styles.singlecontent}>
                <span className={styles.sideheading}>Quốc gia</span>
                <span className={`${styles.conAccent}`}>{data?.countryOfOrigin}</span>
            </div>
            <div className={styles.singlecontent}>
                <span className={styles.sideheading}>Studios</span>
                <span className={`${styles.conAccent}`}>{data?.studios?.nodes?.[0]?.name}</span>
            </div>
            <div className={styles.singlecontent}>
